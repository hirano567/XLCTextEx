...
//	文字を数値に変換するためのデータ
//======================================================================
typedef	PSSearchNumKey<T_UCP, int>	PSCpNumEx;

PSCpNumEx	siCpNumEx[]	=
{
	0x0025,	PUCD_STRTONUM_PERCENT,	//	PERCENT SIGN,
	...
	0xFE6A,	PUCD_STRTONUM_PERCENT,	//	SMALL PERCENT SIGN,
	0xFF05,	PUCD_STRTONUM_PERCENT,	//	FULLWIDTH PERCENT SIGN,
...
size_t	nCpNumEx	= sizeof(siCpNumEx) / sizeof(PSCpNumEx);

//======================================================================
//	CpToIntEx
//======================================================================
inline int CpToIntEx(T_UCP cp)
{
	UINT	uiIndex;

	if (PLib::BinarySearch<PSCpNumEx, UINT>(&uiIndex, siCpNumEx, nCpNumEx, cp))
	{
		return siCpNumEx[uiIndex].Data;
	}
	return (int)(-1);
}
...
bool StrToIntDecUsgn(T_VAL* pVal, T_PCSTR_U* ppszSrc)
{
	T_PCSTR_U	ps	= *ppszSrc;
	int			i;
	T_VAL		v;

	//	一文字目が 10 進数でない場合は false を返す。
	i	= CpToIntEx(*ps);
	if (!PUCD_STRTONUM_IS_DEC(i))
	{
		return false;
	}
	v	= (T_VAL)i;
	...
	try
	{
		...
		{
			i	= CpToIntEx(*ps);
			if (i == PUCD_STRTONUM_COMMA)
			{
				++ps;
				continue;
			}
			if (!PUCD_STRTONUM_IS_DEC(i))	break;
			v	= v * 10 + (T_VAL)i;
			++ps;
		}
	}
...
bool StrToIntDecSgn(T_VAL* pVal, T_PCSTR_U* ppszSrc, int* piSign)
{
	...
	//	一文字目が 10 進数なら符号を正として次へ。
	if (PUCD_STRTONUM_IS_DEC(i))
	{
		*piSign	= 1;
	}
	//	符号ならそれをセットし次の文字を数値に変換する。
	else if (i == PUCD_STRTONUM_PLUS || i == PUCD_STRTONUM_MINUS)
	{
		if (i == PUCD_STRTONUM_MINUS) *piSign = -1; else *piSign = 1;
		++ps;
		i	= CpToIntEx(*ps);
		if (!PUCD_STRTONUM_IS_DEC(i))	return false;
	}
	//	以上に当てはまらない場合は false を返す。
	else
	{
		*piSign	= 0;
		return false;
...
//======================================================================
//	StrToIntDecFmt
//	次の形式の文字列を 10 進の整数とみなして数値に変換する。
//		(通貨記号\s*)?[+|-]?[0-9,]+
//	通貨記号は GetLocaleInfo で取得する。
//	通貨記号と , があればそこで処理を終了させず無視して以降の文字列を処理する。
//	引数のチェックは呼び出し側で行っておくこと。
//======================================================================
template<typename T_VAL>
bool StrToIntDecFmt(T_VAL* pVal, T_PCSTR_U* ppszSrc, int* piSign)
{
	...
	size_t		zSrc		= 0;
	size_t		zSCur		= 0;
	bool		bSCur		= false;	//	通貨記号がすでに使われたか
	int			ir;
	DWORD		dwCSFlags	= (NORM_IGNORECASE | NORM_IGNOREWIDTH);

	*piSign	= 0;

	//	Locale から通貨記号を取得しておく。
	//	取得できなかった場合（バッファ不足も含めて）は「なし」とする。
	wmemset(SCur, (wchar_t)0, (size_t)iSCur);
	ir	= GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SCURRENCY, SCur, iSCur - 1);
	if (ir == 0)
	{
		SCur[0]	= (T_CHAR_U)0;
	}
	...
	//------------------------------------------------------------
	//	数字の前に通貨記号や符号がついていてもよい。
	//------------------------------------------------------------
	i	= CpToIntEx(*ps);
	while (true)
	{
		//	１文字目が符号ならそれをセットし次の文字を数値に変換する。
		if (i == PUCD_STRTONUM_PLUS || i == PUCD_STRTONUM_MINUS)
		{
			//	符号は１度だけ
			if (*piSign != 0)	return false;

			if (i == PUCD_STRTONUM_MINUS) *piSign = -1; else *piSign = 1;
			++ps;
			i	= CpToIntEx(*ps);
		}
		//	対象文字が数字でないなら、通貨記号かどうかを調べる。
		else if (!PUCD_STRTONUM_IS_DEC(i))
		{
			//	通貨記号は１度だけ
			if (bSCur == true)	return false;

			//	通貨記号が取得できなかった場合は終了
			if (zSCur < 1)	return false;
			zSrc	= wcslen(ps);
			if (zSrc <= zSCur)	return false;
			ir	= CompareStringW(LOCALE_USER_DEFAULT, dwCSFlags, ps, (int)zSCur, SCur, (int)zSCur);
			if (ir != CSTR_EQUAL)	return false;

			//	通貨記号を読み飛ばす。
			ps	+= zSCur;
			//	空白文字があるならそれも読み飛ばす。
			while (*ps != NULL && iswspace(*ps))	++ps;
	...
	if (*piSign == 0)	*piSign = 1;

	//------------------------------------------------------------
	//	数字が続く限り取り出して整数値に変換する。
	//------------------------------------------------------------
	//	対象文字が 10 進数でないなら終了。
	if (!PUCD_STRTONUM_IS_DEC(i))	return false;
...
bool StrToDouble(double* pdValue, T_PCSTR_U* ppszSrc)
{
	...
	if (StrToIntDecFmt<double>(&dip, &ps, &isg))
	{
	}
	//	最初が小数点の場合も次の処理へ。
	else if (CpToIntEx(*ps) == PUCD_STRTONUM_POINT)
	{
		//	ここへ来た場合、isg には 0 がセットされている。
		isg	= 1;
	}
	//	+. と -. も受け付ける。
	else if (isg != 0)
	{
		++ps;
		if (CpToIntEx(*ps) != PUCD_STRTONUM_POINT)	return false;
	}
	//	以上に当てはまらない場合は false を返す。
	else
	{
	...
	//	小数部を取得する。
	if (CpToIntEx(*ps) == PUCD_STRTONUM_POINT)
	{
		try
		{
			++ps;
	...
	//	次の文字が % なら得られた値を 1/100 にし、ポインタを進めて返す。
	i	= CpToIntEx(*ps);
	if (i == PUCD_STRTONUM_PERCENT)
	{
		*pdValue	= (dip + dfp * isg) / 100;
		++ps;
		*ppszSrc	= ps;
		return true;
	}
	//	次の文字が E、e でないなら終了する。
	else if (i != PUCD_STRTONUM_EXPONENTIAL)
	{
		*pdValue	= dip + dfp * isg;
		*ppszSrc	= ps;
		return true;
	...
	pt	= ps;
	++ps;
	if (StrToIntDecSgn<int>(&iep, &ps, &itmp))
	{
		i	= CpToIntEx(*ps);
		//	次の文字が % なら指数部から 2 を引いて計算する。ポインタを進めて返す。
		if (i == PUCD_STRTONUM_PERCENT)
		{
			iep	-= 2;
			*pdValue	= (dip + dfp * isg) * pow(10.0, iep);
			++ps;
			*ppszSrc	= ps;
		}
		//	% でない場合
